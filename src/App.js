			...
			<NoteProvider>
				<div className="min-h-screen p-4 bg-slate-900 text-white flex flex-col gap-4">
					...
					<div className="flex justify-around">
						<button className={`rounded-lg px-8 py-4 border ${page === 1 ? 'bg-[rgba(255,255,255,0.07)]' : ''}`} onClick={() => setPage(1)}>1er annee</button>
						<button className={`rounded-lg px-8 py-4 border ${page === 2 ? 'bg-[rgba(255,255,255,0.07)]' : ''}`} onClick={() => setPage(2)}>2eme annee</button>
					</div>
					{page === 1 && <FirstYear />}
					{page === 2 && <SecondYear />}
				</div>
